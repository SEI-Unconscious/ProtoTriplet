...
# ===============================
# Helper: Plot Confusion Matrix (Modified for IEEE)
# ===============================
def plot_confusion_matrix(y_true, y_pred, save_path, class_names):
    """
    绘制并保存混淆矩阵，去除丑陋的刻度线 (Ticks)
    """
    ...
    # 2. 修改 figure 尺寸为 3.5 英寸宽 (IEEE单栏). 高度设为 3.2 尽量紧凑
    plt.figure(figsize=(3.5, 3.2))

    # 绘制热力图
    sns.heatmap(cm_norm, annot=True, fmt=".2f", cmap="Blues",
                xticklabels=class_names, yticklabels=class_names,
                annot_kws={"size": 7}, # 调整格子内数字大小
                cbar_kws={"shrink": .9} # 稍微缩小图例条
               )
    ...
    # ==========================================
    # 关键修改：去除坐标轴上的“小刻度线”(Ticks)
    # ==========================================
    plt.tick_params(axis='both', which='both', length=0)

    plt.tight_layout()
    # 使用 bbox_inches='tight' 确保保存时边缘标签不被截断
...
def fewshot_test(model, test_loader):
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            inputs = inputs.to(device)
            label_tensor = labels[:, 0].to(device) 
...
def main():
    ntx = 7
    nrx = 3
    few_shot_nums = [0, 1, 2, 3, 5, 10, 20]

    # 定义具体的手机名称标签 (0-6)
    class_names = ['honor0', 'honor1', 'nova', 'oppo0', 'oppo1', 'redmi0', 'redmi1']
    ...
    for few_shot in few_shot_nums:
        ...
        acc, y_true, y_pred = fewshot_test(model, test_loader)
        print(f"🎯 Few-shot {few_shot} Test Accuracy: {acc:.2f}%")

        cross_test_accuracys.append(acc)
        
        # 传递 class_names 给绘图函数
        cm_save_path = f"../figures/cm_fewshot_{few_shot}_{current_date}.png"
        plot_confusion_matrix(y_true, y_pred, cm_save_path, class_names=class_names)

    # Save results to CSV in ../results/
    df = pd.DataFrame({
        "few_shot_nums": few_shot_nums,
        "cross_test_accuracys": cross_test_accuracys
    })
    csv_path = f"../results/fewshot_Triplet_My_Data_{current_date}.csv"
