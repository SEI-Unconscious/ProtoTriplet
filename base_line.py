...
def data_load_prepare(data, ntx = 10, nrx = 5):
    data_cross_inputs = []
    data_cross_labels = []
    rxi = nrx
...
def cross_model_train(model, train_cross_loader, val_cross_loader, num_epoch = 50, model_save_path = "./model/", time = "2025_12_23", index = 1):
    ...
    for epoch in range(num_epoch):
        ...
        if val_epoch_loss < best_loss:
            ...
            torch.save(model.state_dict(), f"{model_save_path}best_model_baseline_{time}_{index}.pth")
...
def main():
    ...
    print("===========================")
    data_cross_inputs, data_cross_labels = data_load_prepare(data, ntx=ntx, nrx=nrx)
    ...
    for few_shot_num in few_shot_nums:
        ...
        cross_model = SEINet_Baseline(num_classes=ntx, base_model_path="base_model_resnet18_baseline.pth").to(device)
    ...
    df.to_csv('../results/base_line' + '.csv', index=False)
