...
def main():
    ntx = 7
    nrx = 5
    few_shot_nums = [0, 1, 2, 3, 4, 5, 10, 20]
    ...
    for few_shot in few_shot_nums:
        ...
        model = SEINet_FewShot_Mapping(
            base_model_path="./model/base_protoNCE.pth",
            num_classes=ntx,
            mapping=False
        ).to(device)
        ...
        model = fewshot_train(
            ...
            num_epochs=100,
            lr=1e-3,
            save_path=f"./model/fewshot_best_{few_shot}.pth"
        )
    ...
    df.to_csv(f"../results/fewshot_proNCE_{current_date}.csv", index=False)
